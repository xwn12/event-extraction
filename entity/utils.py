...
def convert_dataset_to_samples(dataset, max_span_length, ner_label2id=None, context_window=0, num_ner_labels=0, split=0):
    """
    ...
    for c, doc in enumerate(dataset):
        ...
        for i, sent in enumerate(doc):
            ...
            # 计算每个实体个数
            for ner in sent.ner:
                num_ner[ner_label2id[ner.label]] +=1
            sample = {
    ...
    logger.info('Extracted %d samples from %d documents, with %d NER labels, %.3f avg input length, %d max length'%(len(samples), data_range[1]-data_range[0], sum(num_ner), avg_length, max_length))
